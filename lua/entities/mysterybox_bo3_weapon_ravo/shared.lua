...
ENT.PrintName 		= "Mystery Box BO3 Weapons Entity"
ENT.Author 			= "ravo Norway"
ENT.Purpose 		= "Shows the model of the weapons in the Mystery Box."
ENT.Spawnable 		= false
...
function ENT:SetupDataTables()
	...
	if SERVER then
		...
		self:SetTeddybearRisk( GetConVar( "bo3ravo_mysterybox_bo3_ravo_teddybearGetChance" ):GetFloat() )
...
function DisablePhysgunning(ply, ent)
	if ent:GetClass():lower() == "mysterybox_bo3_weapon_ravo" then
		return false
	end
end
hook.Add( "PhysgunPickup", "bo3Ravo:DisableTakingTheWeaponInTheMysteryBox", DisablePhysgunning )