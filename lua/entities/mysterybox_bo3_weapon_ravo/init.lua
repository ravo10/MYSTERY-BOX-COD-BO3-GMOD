...
include("shared.lua")
----------- --------------------
--- By: ravo Norway (04.05.2019) - Updated: 13.05.21
--------------------
local defaultDivisionValueGetChance = 3.92307

timer.Create( "Bo3ravo_mysteryBox_loader", 0.3, 0, function()

	if bo3_ravo_mysterybox_allowedWeapons then

		timer.Remove( "Bo3ravo_mysteryBox_loader" )
		
		if not ConVarExists("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance") then
			CreateConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance", ( ( table.Count( bo3_ravo_mysterybox_allowedWeapons ) / defaultDivisionValueGetChance ) * -1), bit.bor( FCVAR_PROTECTED, FCVAR_ARCHIVE ), "How likley it is to get a Teddybear. Lower = Higher risk.")
		end
		if not ConVarExists("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance_TotallyCustomValueAllowed") then
			CreateConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance_TotallyCustomValueAllowed", 0, bit.bor( FCVAR_PROTECTED, FCVAR_ARCHIVE ), "If the system will allow totally Custom Teddy Bear Get Chance Value (it doesn't matter how many times you have used the box).")
		end
...
function ENT:SpawnFunction(pl, tr)
	...
	local ent = ents.Create("mysterybox_bo3_weapon_ravo")
	ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	if GetConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance_TotallyCustomValueAllowed"):GetInt() == 0 then

		local chance = math.Round( ( ( table.Count( bo3_ravo_mysterybox_allowedWeapons ) / defaultDivisionValueGetChance ) * -1 ), 3 )

		if not ConVarExists("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance") then
			CreateConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance", chance, FCVAR_PROTECTED, "How likley it is to get a Teddybear. Lower = Higher risk.")
		else
			-- Reset to normal settings
			GetConVar( "bo3ravo_mysterybox_bo3_ravo_teddybearGetChance" ):SetFloat( chance )
		end
...
function ENT:MaybeAdjustTheChanceToGetATeddybear()
	...
	local newTeddybearRisk = GetConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance"):GetFloat()
	...
	if GetConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance_TotallyCustomValueAllowed"):GetInt() == 0 then
		...
		elseif (
			...
			if newTeddybearRisk == 1 then
				newTeddybearRisk = GetConVar("bo3ravo_mysterybox_bo3_ravo_teddybearGetChance"):GetFloat()
			end
...
function ENT:SwitchWeaponModel( weaponWModel, weaponClass )
	...
	if weaponWModel and self:IsValid() then
...
function ENT:CycleWeapons()
	...
	--
	-- Cycle Creator
	local tableCountAllowedWeapons = table.Count( bo3_ravo_mysterybox_allowedWeapons )
	...
	local function _switchWeapons(self)
		...
		local theWeaponsTable = bo3_ravo_mysterybox_allowedWeapons[ randomWeaponIndex ]
		...
		if self:IsValid() then
			...
			if randomWeaponIndex <= 0 then
				self:SwitchWeaponModel( "models/mysterybox_bo3_ravo/teddybear/mysterybox_bo3_teddybear_standalone_ravo.mdl", nil )
		...
		if (
			...
			net.Start("bo3Ravo:setClientModleMysteryBoxRavo")
				net.WriteTable({
					Weapon = self,
					WModel = _WModel
				})
		...
		elseif (
			...
			------------ ------------------------------------
			if self and self:IsValid() and self:GetParentBoxEntity().TeddybearIncoming then
			...
			timer.Simple(2.3, function()
				...
				local teddyent = ents.Create("mysterybox_bo3_teddybear_ravo")
...
function ENT:Think()
	...
	if self:GetModel() == "models/mysterybox_bo3_ravo/teddybear/mysterybox_bo3_teddybear_standalone_ravo.mdl" then
...
function ENT:Use( activator, caller, useType, value )
	...
	if not Bo3RavoIsPlayerAllowedToActivate( MysteryBox, activator, weaponIsCloseEnough ) then return end
	Bo3RavoMysteryBoxTakeWeapon( MysteryBox, activator, weaponChild )
