...
function ENT:Initialize()
	...
	self:SetModel("models/mysterybox_bo3_ravo/teddybear/mysterybox_bo3_teddybear_standalone_ravo.mdl")
...
function ENT:Think()
	...
	-- Start Position
	local extraZ = self:GetChangingWeaponZPosValue()

	-- Move Weapon UP... . --
	local __zPos = math.max(self:GetPrevWepEntityPos().z, _parentEnt:GetPos().z)
	-- Position
	self:SetPos( Vector( _parentEnt:GetPos().x, _parentEnt:GetPos().y, __zPos + extraZ ) )
	self:SetAngles( _parentEnt:GetAngles() )

	local _newVal = extraZ + __amountUp

	-- Increase
	self:SetChangingWeaponZPosValue(_newVal)
